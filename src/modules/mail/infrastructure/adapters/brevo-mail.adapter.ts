...
export class BrevoMailAdapter implements MailPort {
  ...
  constructor(private readonly config: ConfigService) {
    this.fromEmail = config.get<string>("EMAIL_FROM") ?? "ekandacode@gmail.com";
    this.fromName = config.get<string>("EMAIL_FROM_NAME") ?? "CandongueiroPay";
    this.replyToEmail =
  ...
  private resolveApiKey(): string {
    ...
    if (!key) {
      throw new Error(
        "EMAIL_API_KEY em falta — crie em Brevo → SMTP & API → API keys",
      );
    }
    return key;
  }
    ...
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        replyTo: {
          ...
          name: input.replyTo?.name ?? this.fromName,
        },
